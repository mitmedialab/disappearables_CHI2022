...
void mouseDragged() {
  ...
  if(draggingOriginDest){
   if(draggedOriginorDest == "Destination"){
     cubes[draggedOriginDest].targetx = CanvasToMatVec(new PVector(mx_, my_)).x;
     cubes[draggedOriginDest].targety = CanvasToMatVec(new PVector(mx_, my_)).y;
   } else {
     currentPos[draggedOriginDest] = CanvasToMatVec(new PVector(mx_, my_));
   }
